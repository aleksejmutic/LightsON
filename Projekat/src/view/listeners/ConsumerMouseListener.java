...
public class ConsumerMouseListener extends MouseAdapter {
    private DiagramCanvas canvas;
    private boolean isDrawing = false;
    private Editor editor;
    
    public ConsumerMouseListener(Editor editor) {
    	this.editor = editor;
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            if (isDrawing) {
                drawNewConsumer(clickPoint);
            }
        }
    }

    private void drawNewConsumer(Point clickPoint) {
        int boxWidth = 100;
        int boxHeight = 50;

        if (isOverlappingExistingConsumers(clickPoint.x, clickPoint.y, boxWidth, boxHeight)) {
            Toolkit.getDefaultToolkit().beep();
        } else {
            Consumer consumer = new Consumer(clickPoint, clickPoint, boxWidth, boxHeight);
	...
	private Consumer getClickedConsumers(Point point) {
        for (Element element : canvas.getDiagram().getElements()) { // Iterate through Elements
            if (!(element instanceof CompositeWire)) { // For every element besides the CompositeWire
                Consumer box = (Consumer) element; // Cast to JunctionBox
    ...
    private boolean isOverlappingExistingConsumers(int x, int y, int width, int height) {
