...
public class SocketMouseListener extends MouseAdapter {
    private DiagramCanvas canvas;
    private boolean isDrawing = false;
    private Editor editor;
    
    public SocketMouseListener(Editor editor) {
    	this.editor = editor;
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            System.out.println("Socket clicked");

            if (isDrawing) {
                drawNewSocket(clickPoint);
            }
        }
    }

    private void drawNewSocket(Point clickPoint) {
        int boxWidth = 100;
        int boxHeight = 50;

        if (isOverlappingExistingSockets(clickPoint.x, clickPoint.y, boxWidth, boxHeight)) {
            Toolkit.getDefaultToolkit().beep();
        } else {
            Socket consumer = new Socket(clickPoint, clickPoint, boxWidth, boxHeight);
            Diagram diagram = canvas.getDiagram();
            diagram.addElement(consumer);
	...
	private Socket getClickedSockets(Point point) {
        for (Element element : canvas.getDiagram().getElements()) { // Iterate through Elements
            if (!(element instanceof CompositeWire)) { // For every element besides the CompositeWire
                Socket box = (Socket) element; // Cast to JunctionBox
    ...
    private boolean isOverlappingExistingSockets(int x, int y, int width, int height) {
