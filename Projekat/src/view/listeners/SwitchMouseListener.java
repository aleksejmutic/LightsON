...
public class SwitchMouseListener extends MouseAdapter {
    private DiagramCanvas canvas;
    private boolean isDrawing = false;
    private Editor editor;
    
    public SwitchMouseListener(Editor editor) {
    	this.editor = editor;
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            if (isDrawing) {
                drawNewSwitch(clickPoint);
            }
        }
    }

    private void drawNewSwitch(Point clickPoint) {
        int boxWidth = 100;
        int boxHeight = 50;
        ...
        if (isOverlappingExistingFuses(clickPoint.x, clickPoint.y, boxWidth, boxHeight)) {
            ...
            SwitchElement fuses = new SwitchElement(clickPoint, clickPoint, boxWidth, boxHeight);
            Diagram diagram = canvas.getDiagram();
            diagram.addElement(fuses);
	...
	private SwitchElement getClickedSwitch(Point point) {
        for (Element element : canvas.getDiagram().getElements()) { // Iterate through Elements
            if (!(element instanceof CompositeWire)) { // For every element besides the CompositeWire
                SwitchElement box = (SwitchElement) element; // Cast to JunctionBox
